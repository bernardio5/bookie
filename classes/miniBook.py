...
class miniBook:
    def __init__(self):
        ...
        self.gutenId = 0
        self.valid = False
        self.dir = ""  # dir path to book's contents' dir in final library -- OS-specific; windows
        self.htmlRelativePath = "" # html addr relative to lib root 
        self.brutalTitle = "" # os-path-safe version of title, all caps, 
        self.brutalTitleA = "" # first letter of brutalTitle
        self.brutalTitleAB = "" # first two letters
        self.titlePath = "" # os-specific:  html root + "\\titles\\" + brutalTitleA + '\\'
        self.bookTag = "" # os-path-safe version of fullbook.booktag
        self.volCount = 0 # number of "volumes" generated by textWrapper's processing
    ...
    def arrangeTTYBook(self, fullbook): 
        ...
        if not fullbook.txtPath=="-":
            ...
            tw = textWrapper(fullbook.bookPath, self.bookTag, targetDir) # source, titleIn, targetDir
            self.volCount = tw.output()
            print (str(self.volCount)+ " volumes made")
        else:
            toPt = targetDir + "\\" + fullbook.safeFn(fullbook.bookTag()) + ".htm"
            if os.path.isfile(fromPt):
                copyfile(fromPt, toPt)
                os.remove(fullbook.bookPath)
        ...
        if os.path.isfile(fromPt):
            copyfile(fromPt, toPt)
        # book's HTML root is the last step
    ...
    def copyText(self, fullbook): 
        ...
        if not os.path.exists(self.targetDir):
            os.makedirs(targetDir)
        src_files = os.listdir(self.targetDir) # clear out dir
        for fn in src_files:
            pt = targetDir + "\\" + fn
            if os.path.isfile(pt):
                os.remove(pt)
    ...
    def makeHTML(self):
        ...
        # file.write('<a href="' + self.bookTag+ '.epub"><img src="cover.jpg"/></a><br/>')
        file.write('<a href="' + self.bookTag + '_01.htm"><img src="cover.jpg"/></a><br/>')
        for i in range(1, self.volCount):
            if i<10:
                file.write('<a href="' + self.bookTag + '_0' + str(i) + '.htm">v.' + str(i) + '</a> -- ')
            else:
                file.write('<a href="' + self.bookTag + '_' + str(i) + '.htm">v.' + str(i) + '</a> -- ')
            if i%5==4:
                file.write("<br>") 
        file.write('<br><br>')
        ...
        for tp in self.topics:
            ...
            if not spPl==-1:
                dm = tp[0:spPl]
                file.write('<a href="../../../../topics/' + dm + '.html">' + tp + '</a><br/>')
